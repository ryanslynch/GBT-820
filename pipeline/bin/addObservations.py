#!/usr/bin/env python
import os, sys, numpy, datetime, re
import database
...
def parse_files(filenms):
    ...
    for filenm in filenms:
        ...
        try:
            ...
            hdulist.close()
            pattern = re.compile(re.escape("GBT820-"),re.IGNORECASE))
            d["BeamID"] = int(pattern.sub("",hdr0["SRC_NAME"]))
...
def upload(filenms):
     ...
     for i in info:
         query = "INSERT INTO GBT820 (%s) VALUES (%s)" % \
...
if __name__ == "__main__":
    if sys.argv[1] == "--from-file":
        ...
        filenms = sys.argv[1:]
    db = database.Database("observations")
    query = "SELECT FileName FROM GBT820"
    db.execute(query)
